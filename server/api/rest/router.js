...
router.get('/otSummery', async(ctx, res) => {
   ...
   // console.log(date)
    const filedRecords =  await ctx.model('OvertimeReport').find({periodType:period, startDate: date,$or: [{ status: "ready" }, { status: "approved" }] }).lean().exec()
   const members = await ctx.model('Member').exec()
    const withExtraOt = {
        currentTeam: 'Fixed',
        records: members.filter(m => m.extraOT).map(m => {
            return { other: m.extraOT, totalPayHours: m.extraOT, _id: m._id, autoGenerated: true, member: _.pick(m, ['fullName', 'id', 'mateId']) }
        })
    }
